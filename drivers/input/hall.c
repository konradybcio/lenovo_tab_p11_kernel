...
#define KEY_HALL_OPEN                360
#define KEY_HALL_CLOSE               361
		...
		struct class_attribute *attr, char *buf)
{
	...
	if (global_hall_info->hall_switch_state)
		...
		state = 2;
	pr_err("%s state = %d, custome_state=%d\n", __func__,
...
static int hall_register_class_dev(struct hall_switch_info *hall_info)
{
	...
	if (!hall_info->hall_sys_class) {
		hall_info->hall_sys_class = class_create(THIS_MODULE,
						"hall_class");
...
static int hall_probe(struct platform_device *pdev)
{
	...
	pr_err("%s Hall_sensor hall_probe\n", __func__);

	if (pdev->dev.of_node) {
		hall_info = kzalloc(sizeof(struct hall_switch_info),
					GFP_KERNEL);
		if (!hall_info) {
			pr_err("hall failed to memory for module data\n", __func__);
			return -ENOMEM;
		}
		err = hall_parse_dt(&pdev->dev, hall_info);
		if (err) {
			pr_err("%s DT parsing failed\n", __func__);
			goto free_struct;
		}
	} else{
		return -ENOMEM;
...
/*interrupt config*/
	if (gpio_is_valid(hall_info->irq_gpio)) {
		...
		pr_err("%s Hall_sensor hall irq gpio not provided\n", __func__);
		goto free_input_device;
	}
