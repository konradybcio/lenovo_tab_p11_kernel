		...
		int len, uint8_t *data, uint8_t bank)
{
	...
	if (g_core_fp.fp_register_write(tmp_addr, data_len, w_data, 0)
			== NO_ERR)
		ret = true;
	else
		ret = false;
...
{
	D("%s:addr: 0x%02X%02X%02X%02X, write to:0x%02X%02X%02X%02X\n",
		__func__,
...
//	uint8_t j = 0;
	...
	for (i = 1; i < part_num; i++) {
		...
		D("%s,[%d]SRAM addr=%08X, fw_addr=%08X, write_size=%d\n",
			__func__, i,
	...
	if (G_POWERONOF == 1) {
...
#if 1
		D("Now sram_min[0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
		...
		D("%s: clear handshaking data = %02X%02X%02X%02X\n",
		...
		D("%s: write reply retry = %d, data = 0x%02X%02X%02X%02X\n",
				__func__, retry, recv_data[0], recv_data[1], recv_data[2], recv_data[3]);

		D("%s: waiting for FW reload done", __func__);
		...
		while (retry++ < 30) {
			...
			if ((data[3] == 0x00 && data[2] == 0x00 &&
						data[1] == 0x72 && data[0] == 0xC0)) {
				D("%s: FW finish reload done\n", __func__);
				break;
			}
			D("%s: wait reload done %d times\n", __func__, retry);
			g_core_fp.fp_read_FW_status();
			usleep_range(10000, 11000);
		}
#endif
