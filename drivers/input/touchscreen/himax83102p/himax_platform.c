...
int himax_dev_set(struct himax_ts_data *ts)
{
	...
	ts->hx_pen_dev->id.vendor = 0xbeef;
	ts->hx_pen_dev->id.product = 0xdead;
	ts->hx_pen_dev->id.version = 0x28bb;
skip_pen_operation:
...
/*void switch_screen_off(int enable)
{
	...
	psy = power_supply_get_by_name("mcu_tp");
	if(!psy)
	{
		pr_err("======================mcu_tp psy=======null======\n", __func__);
	}
	else
	{
		power_supply_set_property(psy,POWER_SUPPLY_PROP_screen_status,&reta);
	}

	psy = power_supply_get_by_name("mcu_kbd");
	if(!psy)
	{
		pr_err("======================mcu_kbd psy=======null======\n", __func__);
	}
	else
	{
		power_supply_set_property(psy,POWER_SUPPLY_PROP_screen_status,&reta);
...
struct himax_ts_data *ts_drm_data;
void himax_drm_notify_work(struct work_struct *work)
{
	struct himax_ts_data *ts = ts_drm_data;
...
int himax_drm_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
