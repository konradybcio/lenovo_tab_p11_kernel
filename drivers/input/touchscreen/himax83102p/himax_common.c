...
#define HIMAX_PROC_PEN_FILE "active_pen"
struct proc_dir_entry *himax_proc_pen_file;
...
EXPORT_SYMBOL(Himax_gesture_status);
...
static ssize_t himax_PEN_read(struct file *file, char *buf,
		size_t len, loff_t *pos)
{
	//struct himax_ts_data *ts = private_ts;
	size_t ret = 0;
...
static const struct file_operations himax_proc_Pen_ops = {
	.owner = THIS_MODULE,
	.read = himax_PEN_read,
};
...
int himax_common_proc_init(void)
{
	...
	himax_proc_pen_file = proc_create(HIMAX_PROC_PEN_FILE , 0444,
		himax_touch_proc_dir, &himax_proc_Pen_ops);
	if (himax_proc_pen_file == NULL) {
		E(" %s: proc pen file create failed!\n", __func__);
		goto fail_7;
	}
	return 0;

	remove_proc_entry(HIMAX_PROC_PEN_FILE, himax_touch_proc_dir);
fail_7:
...
/*			0, 0);*/
	...
	set_bit(INPUT_PROP_POINTER, ts->hx_pen_dev->propbit);
...
static void himax_fb_register(struct work_struct *work)
{
	...
	if (active_panel &&
		drm_panel_notifier_register(active_panel,
			&ts->fb_notif) < 0)
		E("register drm notifier failed!\n");
#endif
#endif
	if (ret)
 ...
 */
	...
	ts->himax_drm_notify_wq = create_singlethread_workqueue("HMX_DRM_notify");
	if (!ts->himax_drm_notify_wq) {
		E(" allocate himax_drm_notify_wq failed\n");
		err = -ENOMEM;
		goto err_himax_drm_notify_wq_failed;
	}
	INIT_WORK(&ts->himax_drm_notify, himax_drm_notify_work);
#endif
	...
	hq_regiser_hw_info(HWID_LCM, "Vendor:BOE\nIC:himax83102P 1200H*2000V 60fps\nLCD_FW:00");
	hq_regiser_hw_info(HWID_CTP, "Vendor:BOE\nIC:himax83102P Report Rate=120HZ TouchNum=10\nTP_FW:09");
	return 0;
...
err_creat_proc_file_failed:
	...
	destroy_workqueue(ts->himax_drm_notify_wq);
err_himax_drm_notify_wq_failed:
