...
static int sar_parse_dt(struct device *dev)
{
	...
	int I2C_BASE_OFFSET = 1167;

	sar_int_gpio = of_get_named_gpio(np, "vituralsar,irq-gpio", 0);
	printk("[%s]: sar_int_gpio = %d\n ", __func__, sar_int_gpio);
	if ((sar_int_gpio - I2C_BASE_OFFSET) == 39) {
		printk("[%s]: parse irq gpio correctly\n ", __func__);
		return 0;
	} else {
		printk("[%s]: parse irq gpio incorrectly\n ", __func__);
		return -ENOMEM;
	}
}
...
static irqreturn_t sar_irq_handler(int irq, void *dev_id)
{
	...
	printk("[%s]: input_report_key ,status :%d\n", __func__, input_data);
...
static int virtualsar_probe (struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_debug("Enter to  %s do DT matched...\n", __func__);
