...
#define RT1711H_DRV_VERSION	"2.0.3_G"

#define RT1711H_IRQ_WAKE_TIME	(500) /* ms */


extern bool has_rt1711;
				...
				const struct i2c_device_id *id)
{
	...
	if(is_power_off_charge())
	{
		pr_err("==========power_off_charger do not load rt1711 adriver======\r\n");
		has_rt1711 = false;
		return -ENODEV;
	}
	...
	if (!pogo_psy) {
		pr_info("==================RT1711 - %s Could not get USB power_supply, deferring probe\n",
			__func__);
		return -EPROBE_DEFER;
	}
