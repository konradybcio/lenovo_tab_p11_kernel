...
#endif
void disable_usb_host(struct fusb30x_chip* chip)
{
	...
	pr_err("======================FUSB - %s\n", __func__);
	reta.intval = 0;
	power_supply_set_property(chip->pogo_psy,POWER_SUPPLY_PROP_usb_host_ctl,&reta);
}

void enable_usb_host(struct fusb30x_chip* chip, bool ss)
{
        union power_supply_propval reta = {0,};
		...
		void *usr_ctx, void *app_ctx)
{
	...
	case DATA_ROLE:
		...
		break;
	case POWER_ROLE:
		pr_info("FUSB - %s:POWER_ROLE=0x%x", __func__, event);
