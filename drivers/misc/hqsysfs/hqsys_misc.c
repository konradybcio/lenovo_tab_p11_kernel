...
int get_data_from_vfs(char *qcom_path,int qcom_path_len,char *buf_size){
	struct file *pfile = NULL;
	mm_segment_t old_fs;
	loff_t pos;
	ssize_t ret = 0;

	pfile = filp_open(qcom_path,O_RDONLY,0);
	if (IS_ERR(pfile)) {
	    return -1;
	}
	...
	ret = vfs_read(pfile, buf_size, qcom_path_len,&pos);
	if (IS_ERR(pfile)) {
	    goto out;
	}
	set_fs(old_fs);
...
char ufs_vendor_name[30];
char ufs_module_name[30];

void get_hwinfo_emmc_info(hwinfo_emmc *hwinfo_emmc_info){
	#define K(x) ((x) << (PAGE_SHIFT - 10))
	struct sysinfo i;
	char *buf_size = (char*)kzalloc(qcom_emmc_len,GFP_KERNEL);
	//unsigned long long Size_emmc=0;
	si_meminfo(&i);
	hwinfo_emmc_info->ddr_size = round_kbytes_to_readable_mbytes(K(i.totalram))/1024;

	printk(" ufs_vendor_name:%s  len:%d\n ",   ufs_vendor_name, strlen( ufs_vendor_name) );
	printk(" ufs_module_name:%s len:%d\n ",   ufs_module_name,strlen( ufs_module_name) );
	if(strlen( ufs_vendor_name)!=0)
	{
		strcpy(hwinfo_emmc_info->vendor_name, ufs_vendor_name);
		strcpy(hwinfo_emmc_info->emmc_name, ufs_module_name);
	}
	else
	{
		printk(" unknow ufs \n ");
		strcpy(hwinfo_emmc_info->vendor_name, "unknow vendor");
		strcpy(hwinfo_emmc_info->emmc_name, "unknow model");
	}

	/*
	get_data_from_vfs(qcom_emmc,qcom_emmc_len,buf_size);
	Size_emmc = simple_strtoull(buf_size,NULL,0)>>1;

	hwinfo_emmc_info->emmc_size = round_kbytes_to_readable_mbytes((unsigned int)Size_emmc)/1024;
	memset(buf_size,0,qcom_emmc_len);
	get_data_from_vfs(qcom_emmc_name,qcom_emmc_len,buf_size);
	strcpy(hwinfo_emmc_info->emmc_name, buf_size);
	memset(buf_size,0,qcom_emmc_len);
	get_data_from_vfs(qcom_vendor_name,qcom_emmc_len,buf_size);
	Size_emmc = simple_strtoull(buf_size,NULL,0);
	if(0x15 == Size_emmc){
		strcpy(hwinfo_emmc_info->vendor_name, "Samsung");
	}else if(0x11 == Size_emmc){
		strcpy(hwinfo_emmc_info->vendor_name, "Toshiba");
	}else if(0x90 == Size_emmc){
		strcpy(hwinfo_emmc_info->vendor_name, "Hynix");
	}else if(0x13 == Size_emmc){
		strcpy(hwinfo_emmc_info->vendor_name, "Micron");
	}else if(0x45 == Size_emmc){
		strcpy(hwinfo_emmc_info->vendor_name, "SanDisk");
	}else{
		strcpy(hwinfo_emmc_info->vendor_name, "Unknown");
	}
	*/
